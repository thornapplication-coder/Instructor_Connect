...
export function StandardisationReport({
  ...
  const scoped = useMemo(() => scopeRecords(records, { fleet, period, now }), [records, fleet, period, now])
  ...
  const exportCsv = () => {
    ...
    csv += row(['Period', label, 'Fleet', fleet || 'All fleets'])
    ...
    <div className="space-y-3 print-landscape">
      ...
      <div className="border-b-2 border-line/60 pb-2">
        ...
        <p className="mt-1 flex flex-wrap justify-between gap-x-4 gap-y-0.5 text-[11px] text-dim">
          <span>
            {t('grading.std.period.label')}: {label} · {t('grading.admin.fleet')}: {fleet || t('grading.admin.allAircraft')}
          </span>
