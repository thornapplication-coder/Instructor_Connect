...
- Grading Tool: digital assessment forms per OM Appendix 5 (grading sheets, additional training, record of attendance, deferred item list). Forms are always in English. Signatures are given live on the device only and are never stored or reused; once signed by both parties a form is locked read-only. Completed forms are emailed automatically to the configured recipients — the Deferred Item List (Form 310) always additionally to the Training Admin. Forms remain visible in your own view for one week; in the admin panel they are kept without limit. Printouts and exports always carry date, time and name of the exporting person.
